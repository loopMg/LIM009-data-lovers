...
let newArrFilter = '';
containerButtomsType.addEventListener('click', (ee) => {
  newArrFilter = window.filteredPokemons(dataPokemon, ee.target.value);
  let string = '';
...
buttomResult.addEventListener('click', () => {
  ...
  let result = window.countingPoke(dataPokemon, parsear);
